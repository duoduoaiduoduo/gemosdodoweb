...
export async function preparePhoto(photo){
 ...
 canvas=document.createElement('canvas');canvas.width=canvas.height=1536;
 const ctx=canvas.getContext('2d',{willReadFrequently:true});if(!ctx)throw Error('手机无法分配图片处理内存，请关闭其他标签后重试。');
 ctx.fillStyle='#fff';ctx.fillRect(0,0,1536,1536);ctx.drawImage(image,0,0,1536,1536);
 return {width,height,pixels:ctx.getImageData(0,0,1536,1536).data};
