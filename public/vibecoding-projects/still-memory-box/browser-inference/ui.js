import {memoryBox} from '../main.js?v=scale65-20260913-1';
...
$('local-example').onclick=async()=>{try{const blob=await(await fetch('./photo.jpg?v=scale65-20260913-1')).blob();dialog.close();await create(new File([blob],'站内人潮 · 一刻记忆.jpg',{type:'image/jpeg'}));}catch(e){notice(e.message);sidebar(true);}};
