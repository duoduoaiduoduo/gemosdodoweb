...
export async function applyBakedLighting(group,floor){
 const [metaResponse,uvResponse]=await Promise.all([fetch('./baked/manifest.json?v=cube-20260912-2'),fetch('./baked/lightmap-uv.bin?v=cube-20260912-2')]);
 ...
 const [map,groundMap]=await Promise.all([new THREE.TextureLoader().loadAsync('./baked/irradiance.png?v=cube-20260912-2'),new THREE.TextureLoader().loadAsync('./baked/ground.png?v=cube-20260912-2')]);
