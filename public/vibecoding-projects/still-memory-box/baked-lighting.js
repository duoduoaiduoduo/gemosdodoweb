...
export async function applyBakedLighting(group,floor){
 const [metaResponse,uvResponse]=await Promise.all([fetch('./baked/manifest.json?v=idle-20260913-1'),fetch('./baked/lightmap-uv.bin?v=idle-20260913-1')]);
 ...
 const [map,groundMap]=await Promise.all([new THREE.TextureLoader().loadAsync('./baked/irradiance.png?v=idle-20260913-1'),new THREE.TextureLoader().loadAsync('./baked/ground.png?v=idle-20260913-1')]);
