...
export async function applyBakedLighting(group,floor){
 const [metaResponse,uvResponse]=await Promise.all([fetch('./baked/manifest.json'),fetch('./baked/lightmap-uv.bin')]);
 ...
 const [map,groundMap]=await Promise.all([new THREE.TextureLoader().loadAsync('./baked/irradiance.png'),new THREE.TextureLoader().loadAsync('./baked/ground.png')]);
