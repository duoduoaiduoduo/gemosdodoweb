...
export function createDeskField(scene,keys){
 ...
 pen.rotation.set(0,.2,Math.PI/2);pen.position.set(2.65,-1.707,.85);group.add(pen);
 const ring=new THREE.Group();part(ring,new THREE.TorusGeometry(.13,.018,10,36),silver).rotation.x=Math.PI/2;ring.position.set(2.35,-1.736,-.05);group.add(ring);
 const eraser=new THREE.Group();const e=part(eraser,new THREE.BoxGeometry(.23,.10,.15),rubber);e.rotation.y=.2;eraser.position.set(2.04,-1.705,1.37);group.add(eraser);
 ...
 function update(t,phase){
  ...
  objects.forEach((o,i)=>{
   if(active){const lift=smooth((t-start)/(1.4+i*.22)),a=t*(.65+i*.13)+i*2;
    o.mesh.position.set(o.rest.x+lift*(Math.sin(a)*.075+Math.sin(a*1.7)*.025),o.rest.y+lift*(.48+i*.14+Math.sin(a*1.2)*.055),o.rest.z+lift*Math.cos(a*.8)*.065);
