...
export function filmView(t){
 const keys=[[0,.62,.28,.90],[4,-.32,.20,1.04],[9,.04,.13,1.38],[14,.92,.24,1.12],[18,.56,.28,.92],[20,.56,.28,.92]];
 ...
 return {azimuth:a[1]+(b[1]-a[1])*u,elevation:a[2]+(b[2]-a[2])*u,zoom:a[3]+(b[3]-a[3])*u};
}
...
$('film-start').onclick=async()=>{
 ...
 await new Promise((resolve,reject)=>{rejectRun=reject;cancel=()=>{aborted=true;reject(Error('渲染已取消，记忆保持不变。'));};recorder.ondataavailable=e=>{if(e.data.size)chunks.push(e.data);};recorder.onerror=()=>reject(Error('视频编码失败，请降低设备负担后重试'));recorder.onstop=resolve;recorder.start(1000);const start=performance.now();let last=-100;function tick(now){try{const t=Math.min(20,(now-start)/1000);if(now-last>=32){draw(t);last=now;$('film-status').textContent=`正在渲染 ${Math.round(t/20*100)}% · 请保持页面打开`;}if(t>=20){recorder.stop();return;}raf=requestAnimationFrame(tick);}catch(e){reject(e);}}raf=requestAnimationFrame(tick);timer=setTimeout(()=>reject(Error('录制超时，请保持页面在前台后重试')),45000);document.addEventListener('visibilitychange',hidden);});
