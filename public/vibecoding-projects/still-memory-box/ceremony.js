...
export function createCeremony(scene,inside,camera,stage){
 let card=null,start=0,resolveArrival=null,epoch=0,active=false,phase='idle',revealStart=0,revealing=null;
 ...
 const mat=new THREE.ShaderMaterial({transparent:true,depthWrite:false,uniforms:{time:{value:0},fade:{value:1}},vertexShader:`varying vec2 vUv;void main(){vUv=uv;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}`,fragmentShader:`varying vec2 vUv;uniform float time,fade;
 ...
 const initial=new THREE.Vector3(),initialQ=new THREE.Quaternion(),slotQ=new THREE.Quaternion().setFromEuler(new THREE.Euler(-Math.PI/2-.14,0,0));
 let initialScale=1;
 ...
 function cancel(){epoch++;active=false;waiting.visible=false;disposeCard();if(revealing)revealing.material.uniforms.reveal.value=1;revealing=null;resolveArrival?.();resolveArrival=null;state('idle','');}
 function wait(text='正在重建这一刻'){active=true;waiting.visible=true;mat.uniforms.fade.value=1;state('generating',text);}
 async function begin(file,name){
  cancel();const ticket=epoch;active=true;state('card','将这一刻，装入记忆');const url=URL.createObjectURL(file);
  ...
  material.onBeforeCompile=shader=>{shader.uniforms.swallow={value:0};material.userData.shader=shader;shader.vertexShader=shader.vertexShader.replace('#include <common>','#include <common>\nvarying vec3 cardWorld;').replace('#include <begin_vertex>','#include <begin_vertex>\ncardWorld=(modelMatrix*vec4(position,1.)).xyz;');shader.fragmentShader=shader.fragmentShader.replace('#include <common>','#include <common>\nvarying vec3 cardWorld;uniform float swallow;').replace('#include <clipping_planes_fragment>','#include <clipping_planes_fragment>\nif(swallow>.5 && cardWorld.z<.83)discard;');};
 ...
 function reveal(mesh){waiting.visible=true;revealing=mesh;revealStart=performance.now();mesh.material.uniforms.reveal.value=0;state('revealing','记忆，正在浮现');active=true;}
 function update(now){
  mat.uniforms.time.value=reduced?0:now/1000;
  if(card){const t=(now-start)/1000,hold=reduced?.3:2.4,travel=reduced?.2:1.6,insert=reduced?.2:1.3;
   if(t<hold){card.scale.setScalar(initialScale*(.92+.08*smooth(t/.65)));}
   else if(t<hold+travel){const k=smooth((t-hold)/travel);card.position.lerpVectors(initial,new THREE.Vector3(.98,-.958,1.64),k);card.quaternion.slerpQuaternions(initialQ,slotQ,k);card.scale.setScalar(THREE.MathUtils.lerp(initialScale,.71,k));state('inserting','正在存入这一刻');}
   else {const k=smooth((t-hold-travel)/insert);card.position.set(.98,-.958,THREE.MathUtils.lerp(1.64,.35,k));if(card.material.userData.shader)card.material.userData.shader.uniforms.swallow.value=1;if(k===1){disposeCard();wait();resolveArrival?.();resolveArrival=null;}}
  }
  if(revealing){const p=Math.min(1,(now-revealStart)/(reduced?350:5200));revealing.material.uniforms.reveal.value=p;mat.uniforms.fade.value=1-smooth(p*2);if(p===1){revealing=null;active=false;waiting.visible=false;state('complete','这一刻，已收藏');setTimeout(()=>{if(phase==='complete')state('idle','');},2400);}}
 }
