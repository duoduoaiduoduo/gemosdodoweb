...
export function stillCaseMarkup(lang: string) {
  ...
  return `<article class="still-case">
    ...
    <section class="sc-section"><div class="sc-section-heading"><p class="sc-eyebrow">03 / MADE TO BE SHARED</p><h3>${t('把整个过程，<br>变成一段可以分享的影片。', 'Turn the experience<br>into a film to share.')}</h3><p>${t('从卡片入仓到场景显现，用自动运镜与钢琴配乐，把这次收藏完整地讲给别人看。', 'From the arrival of the card to the reveal of the scene, camera choreography and a piano score tell the story of your memory.')}</p></div>
      <video class="sc-film" src="/gemos-still/assets/intro.mp4" poster="${asset('hero')}" controls playsinline preload="none" aria-label="${t('播放 Gemos Still 展示影片', 'Play the Gemos Still showcase')}"></video>
      ...
      <p class="sc-caption">${t('这里展示的是原生影片的网页压缩版。影片主场景由本机渲染；九宫格片尾使用内置示例素材，4K 导出不会增加原素材细节。', 'This is a compressed web edit of the native film. The main scene is rendered locally; the nine-panel outro uses bundled examples. A 4K export does not add detail to those source clips.')}</p>
    </section>
